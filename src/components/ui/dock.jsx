  ...
  return (
    ...
    >
      {Children.map(children, (child) =>
        isValidElement(child) ? cloneElement(child, { width, isHovered }) : child
      )}
    </motion.div>
  );
}
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          className={cn(
            'absolute -top-6 left-1/2 w-fit whitespace-pre rounded-md border border-emerald-500/30 bg-[#00241b] px-2 py-0.5 text-xs text-emerald-100 shadow-lg pointer-events-none z-50',
            className
          )}
          role='tooltip'
          style={{ x: '-50%' }}
